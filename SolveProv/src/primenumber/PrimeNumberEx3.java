...
public class PrimeNumberEx3 {

public static void main(String[] args) {
		
		int val[];  // main에서 배열타입의 레퍼런스 변수 만드는 과정
		
		val = input_num(); // 전달할 데이터가 없으므로 파라미터 생략. 겍체의 주소를 전달받고 지역변수의 val은 사라지고 객체만 남음
		int cnt_hap[] = process_primenumber(val); //호출(반환값 cnt 있음)
		print_cnt(cnt_hap); //호출
		...
		for(i = val[0]; i <= val[1] ; i++) { //소수 계산 부분 시작
			...
			if(i == j) {  //i변수의 값이 소수인 경우
				...
				cnt_hap[0]++;  //소수 갯수 카운트
				if(cnt_hap[0] % 10 == 0)  //한줄에 10개씩 출력
					System.out.println();
				cnt_hap[1] += i;  // 소수의 합을 구함
				}
			} // 소수 계산 끝
		return cnt_hap;
